...
import {
  ...
  AUTH_MAGIC_LINK_GENERIC_SUCCESS,
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  let payload: AdminMagicLinkPayload;
  try {
    payload = (await request.json()) as AdminMagicLinkPayload;
  } catch {
    return json({ ok: false, message: 'Ongeldige JSON-payload.' }, 400);
  }

  const email = typeof payload.email === 'string' ? payload.email.trim() : '';
  const nextPath = sanitizeAdminRedirectPath(typeof payload.next === 'string' ? payload.next : undefined);
  ...
  try {
    ...
    if (nextPath) {
      headers.append(
        'Set-Cookie',
        `${AUTH_ADMIN_NEXT_COOKIE}=${encodeURIComponent(nextPath)}; Path=/; Max-Age=1800; SameSite=Lax${new URL(request.url).protocol === 'https:' ? '; Secure' : ''}`,
      );
    }
    ...
    const body: Record<string, unknown> = {
      ok: true,
      emailSent,
      message: AUTH_MAGIC_LINK_GENERIC_SUCCESS,
      redirect: `${AUTH_ROUTES.adminCheckEmail}?${redirectParams.toString()}`,
    };

    if (import.meta.env.DEV && !emailSent && !suppressed && plainToken) {
      const devMagicUrl = buildMagicLinkUrl(origin, plainToken, AUTH_ROUTES.adminMagic);
      console.info('[auth/dev] Admin magic link (e-mail niet verzonden):', devMagicUrl);
      body.devMagicUrl = devMagicUrl;
      body.devNotice =
        'Lokale ontwikkelmodus: e-mail is niet verzonden. Gebruik de admin-inloglink op de volgende pagina om te testen.';
    }
